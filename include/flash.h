...
typedef struct {
	__u64	size;			/* total bank size in bytes		*/
	__u16	sector_count;		/* number of erase units		*/
	__u64	flash_id;		/* combined device & manufacturer code	*/
	__u64	start[CONFIG_SYS_MAX_FLASH_SECT];   /* virtual sector start address */
	__u8	protect[CONFIG_SYS_MAX_FLASH_SECT]; /* sector protection status	*/
#ifdef CONFIG_SYS_FLASH_CFI
	__u8	portwidth;		/* the width of the port		*/
	__u8	chipwidth;		/* the width of the chip		*/
	__u16	buffer_size;		/* # of bytes in write buffer		*/
	__u64	erase_blk_tout;		/* maximum block erase timeout		*/
	__u64	write_tout;		/* maximum write timeout		*/
	__u64	buffer_write_tout;	/* maximum buffer write timeout		*/
	__u16	vendor;			/* the primary vendor id		*/
	__u16	cmd_reset;		/* vendor specific reset command	*/
	__u16	interface;		/* used for x8/x16 adjustments		*/
	__u16	legacy_unlock;		/* support Intel legacy (un)locking	*/
	__u16	manufacturer_id;	/* manufacturer id			*/
	__u16	device_id;		/* device id				*/
	__u16	device_id2;		/* extended device id			*/
	__u16	ext_addr;		/* extended query table address		*/
	__u16	cfi_version;		/* cfi version				*/
	__u16	cfi_offset;		/* offset for cfi query			*/
	__u64   addr_unlock1;		/* unlock address 1 for AMD flash roms  */
	__u64   addr_unlock2;		/* unlock address 2 for AMD flash roms  */
...
extern int flash_sect_erase (__u64 addr_first, __u64 addr_last);
extern int flash_sect_protect (int flag, __u64 addr_first, __u64 addr_last);
extern int flash_sect_roundb (__u64 *addr);
extern unsigned long flash_sector_size(flash_info_t *info, flash_sect_t sect);

/* common/flash.c */
extern void flash_protect (int flag, __u64 from, __u64 to, flash_info_t *info);
extern int flash_write (char *, __u64, __u64);
extern flash_info_t *addr2info (__u64);
extern int write_buff (flash_info_t *info, __u8 *src, __u64 addr, __u64 cnt);
...
extern flash_info_t *flash_get_info(__u64 base);
#endif
