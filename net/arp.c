...
unchar	       *NetArpWaitPacketMAC;
int		NetArpWaitTxPacketSize;
ulong		NetArpWaitTimerStart;
int		NetArpWaitTry;

static unchar   *NetArpTxPacket;	/* THE ARP transmit packet */
static unchar	NetArpPacketBuf[PKTSIZE_ALIGN + PKTALIGN];
...
void ArpReceive(struct ethernet_hdr *et, struct ip_udp_hdr *ip, int len)
{
	...
	case ARPOP_REQUEST:
		...
		if ((NetReadIP(&arp->ar_tpa) & NetOurSubnetMask) !=
			...
			udelay(5000);
#endif
		NetSendPacket((unchar *)et, eth_hdr_size + ARP_HDR_SIZE);
		return;
	...
	case ARPOP_REPLY:		/* arp reply */
		...
		if (reply_ip_addr == NetArpWaitReplyIP) {
			...
			net_get_arp_handler()((unchar *)arp, 0, reply_ip_addr,
				0, len);
