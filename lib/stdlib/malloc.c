...
#if 0
static uint8_t mem_malloc_pool[CONFIG_LIB_MALLOC_SIZE] __attribute__ ((aligned (8)));
#else
extern void * const _heap;
extern void * const _stack_end;
#endif

void mem_malloc_init(void)
{
#if 0
	mem_malloc_start = (ulong)mem_malloc_pool;
	mem_malloc_end   = (ulong)mem_malloc_pool + CONFIG_LIB_MALLOC_SIZE;
#else
	mem_malloc_start = (ulong) _heap;
	mem_malloc_end = (ulong) _stack_end;
#endif
	mem_malloc_brk   = mem_malloc_start;

	prints(" Heap allocation range 0x%x - 0x%x(total %d Mb)\n",
		mem_malloc_start, mem_malloc_end, (mem_malloc_end - mem_malloc_start) >> 20);
}
